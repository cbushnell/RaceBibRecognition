...
- **OCR Detection**: Uses Florence-2 for accurate text detection and bib number recognition
...
python main.py /path/to/photos --min-face-confidence 0.9 --min-bib-confidence 0.7

# Only tag bib numbers associated with faces (exclude standalone bibs)
python main.py /path/to/photos --only-associated-bibs

# Skip cleanup of temporary files
python main.py /path/to/photos --no-cleanup
```
